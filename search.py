...
def find_paper_info(soup, key_words, only_title=False):
    res = {}
    ...
    for idx, item in enumerate(dt_list):
        ...
        print('[%d]: %s' % (idx+1, paper_id))
        new_soup = BeautifulSoup(requests.get(
            'https://arxiv.org/abs/'+paper_id).text, 'lxml')
        title = new_soup.find(class_='title mathjax').contents[1]
        abstract = new_soup.find(class_='abstract mathjax').contents[1]
        # papers not contain comment
        if not new_soup.find(class_='tablecell comments mathjax') is None:
            comment = new_soup.find(
                class_='tablecell comments mathjax').contents[0]
        else:
            comment = ''
        flag = True
        for k in key_words:
            if only_title:
                ...
                if not k in title+abstract+comment:
                    flag = False
        if flag:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--only_title', action='store_true',
                        help='search titles only')
    ...
    for date in query_dates:
        ...
        tmp_dict = find_paper_info(soup, query_keys, arg.only_title)
